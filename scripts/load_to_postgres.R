...
get_env <- function(name) {
  ...
  if (!nzchar(value)) stop(sprintf("Missing env var: %s", name))
  value
}
...
raw_file <- get_latest_file(file.path("data", "raw"), "^fm_raw_.*\\.(xlsx|xls|csv)$")
dwc_file <- get_latest_file(file.path("data", "dwc"), "^occurrence_.*\\.csv$")
...
raw_df <- if (grepl("\\.csv$", raw_file, ignore.case = TRUE)) {
  ...
  as.data.frame(readxl::read_excel(raw_file, col_types = "text"),
...
if (!DBI::dbIsValid(con)) stop("PostgreSQL connection is not valid.")
...
# Keeps: pipeline outputs from earlier steps (fm_raw, dwc, etc.)

rm(get_env, get_latest_file, raw_file, dwc_file, raw_df, dwc_df, con, raw_n, dwc_n)
